...
class DiffForcingWanModel(nn.Module):
    def __init__(
        ...
        self_forcing_step_loss_weights=(1, 1, 1, 1, 1),
        ...
        self.self_forcing_step_loss_weights = [
            float(w) for w in self_forcing_step_loss_weights
        ]
        self.self_forcing_start_step = (
            None if self_forcing_start_step is None else int(self_forcing_start_step)
        )
        ...
        if len(self.self_forcing_step_loss_weights) == 0:
            raise ValueError("self_forcing_step_loss_weights must not be empty")
    ...
    def _resolve_self_forcing_k(self, progress):
        k = int(self.self_forcing_k_schedule[0][1])
        for threshold, candidate_k in self.self_forcing_k_schedule:
    ...
    def _resolve_self_forcing_weights(self, k):
        if k > len(self.self_forcing_step_loss_weights):
            raise ValueError(
                f"Need at least {k} self_forcing_step_loss_weights, got "
                f"{len(self.self_forcing_step_loss_weights)}"
            )
        return self.self_forcing_step_loss_weights[:k]

    def _prepare_self_forcing_plan(self, feature_length, device, progress):
        target_k = self._resolve_self_forcing_k(progress)
        min_k_supported = int(feature_length.min().item()) - self.chunk_size + 1
        ...
        phase_offset = float(
            torch.empty(1, device=device).uniform_(0.0, 1.0 / self.chunk_size).item()
        )
        return effective_k, step_weights, start_end_indices, phase_offset
