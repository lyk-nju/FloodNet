...
def test_wan_model_keeps_longer_traj_segment_by_shape():
...
def test_wan_model_defaults_missing_traj_lens_to_full_traj_length():
    model = _tiny_wan_model()
    model(
        [torch.randn(4, 3, 1, 1)],
        torch.zeros(1),
...
def test_wan_model_threads_traj_token_mask_to_attention_by_default():
    model = _tiny_wan_model()
...
def test_wan_cross_attention_frame_aligned_updates_only_latent_segment_when_traj_is_longer(monkeypatch):
    def fake_flash_attention(q, k, v, **kwargs):
        return torch.ones(q.shape[0], q.shape[1], q.shape[2], v.shape[-1])

    monkeypatch.setattr(
        "models.tools.wan_model.flash_attention",
    ...
    out = cross(x, context, context_lens)
...
def test_wan_controlnet_keeps_longer_traj_segment_by_shape():
    cn = WanControlNet(
        model_type="t2v",
        in_dim=4,
        dim=12,
...
def test_wan_controlnet_threads_traj_token_mask_to_attention_by_default():
    cn = WanControlNet(
        model_type="t2v",
        in_dim=4,
        dim=12,
