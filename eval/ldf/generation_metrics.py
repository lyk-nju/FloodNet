...
def _remove_traj_fields(batch: Dict) -> Dict:
    ...
    traj_loss_mask) — otherwise SampleCreator would still route the 7D
    ...
    traj_keys = {
        ...
        # 7D pipeline keys consumed by utils.training.ldf.sample_creator:
...
def _load_model(cfg, ckpt_path: str, device: torch.device, use_ema: bool):
    ...
    # strict=False only for base pretrain warm-start checkpoints that do not
    # contain ControlNet weights. Legacy 4D trajectory checkpoints are no longer
    # auto-migrated on the 7D LDF path.
    strict = has_controlnet
    ...
    if not strict:
        ...
        # missing" is WRONG: missing keys may come from model evolution in an
        # otherwise trained checkpoint. Mirror train_ldf.py's gate.
...
def main():
    ...
    save_dir = Path(args.out_dir) if args.out_dir else _default_output_dir()
        ...
        for i in range(batch_size_actual):
            ...
            for run_idx in range(args.num_runs):
                ...
                if run_idx == 0:
                    ...
                    if "traj_cond_7d" in sample_batch:
                        cond = sample_batch["traj_cond_7d"][0]
                        if torch.is_tensor(cond):
                            cond = cond.detach().cpu().numpy()
                        cond = np.asarray(cond)
                        if cond.ndim == 2 and cond.shape[1] >= 3:
                            np.save(
                                dirs["cond_traj"] / f"{sample_name}.npy",
                                cond[:, [0, 2]].astype(np.float32),
                            )
                    elif "traj_features" in sample_batch:
                ...
                if args.traj_ablation:
                    try:
                        with torch.no_grad():
                            mb_no_traj = prepare_ldf_eval_model_batch(
