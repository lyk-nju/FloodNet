...
from utils.training.noise_initializer.shadow_rollout import inject_context_frontier_zT
...
def slice_future_traj_frames(
    ...
    *,
    ...
    start_frame = max(0, int(commit_index) * int(frames_per_token))
    frame_count = int(traj_horizon_tokens) * int(frames_per_token) + 1
...
def _decode_latents_to_root_xz(vae, latents: torch.Tensor) -> torch.Tensor:
    from eval.ldf.latent_initializer.optimize_noise import _root_xz_from_feature

    vae.clear_cache()
    decoded = vae.decode(latents)[0].float()
    return _root_xz_from_feature(decoded)
...
def run_single_sample_overfit(cfg: dict) -> dict:
    ...
    sample_batch = _load_single_sample(args, ldf_cfg)
    ...
    initializer = NoiseInitializer(**dict(cfg["model"]["params"])).to(device)
    ...
    for commit_index in range(max_commits):
        ...
        if should_train_commit(commit_index, optimize_every_tokens=optimize_every_tokens):
            for inner_step in range(train_steps_per_commit):
                ...
                start_frame = commit_index * frames_per_token
                horizon_frames = int(cfg.get("loss_horizon_tokens", 20)) * frames_per_token
                batch = {
                    "context": context,
                    "target_xz": target_xz[start_frame : start_frame + horizon_frames],
                    "target_mask": target_mask[start_frame : start_frame + horizon_frames],
                    "history_frames": 0,
    ...
    summary = {
        ...
        "completed_commits": int(max_commits),
        "loss_curve": rows,
