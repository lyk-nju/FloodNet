...
def _prepare_noisy_window(model, clean_feature, feature_length, time_steps):
    ...
    for i in range(batch_size):
        end_index = int(model.chunk_size * time_steps[i].item()) + 1
...
def run_training_window(
    ...
    for b in range(batch_size):
        ...
        if model.prediction_type == "vel":
            ...
            squared_error = (
                predicted_result[b][:, -model.chunk_size :, ...]
                - vel[:, -model.chunk_size :, ...]
            ) ** 2
        elif model.prediction_type == "x0":
            squared_error = (
                predicted_result[b][:, -model.chunk_size :, ...]
                - feature_ref[b][:, -model.chunk_size :, ...]
            ) ** 2
        elif model.prediction_type == "noise":
            squared_error = (
                predicted_result[b][:, -model.chunk_size :, ...]
                - noise_ref[b][:, -model.chunk_size :, ...]
            ) ** 2
...
def run_model_step(model, batch) -> dict:
    feature = batch["feature"]  # (B, T, C)
    feature_length = batch["feature_length"]  # (B,)
