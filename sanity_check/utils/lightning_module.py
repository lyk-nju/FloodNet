...
class BasicLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self._ema_updated_this_batch = False
    ...
    def on_save_checkpoint(self, checkpoint):
        if self._ema_updated_this_batch:
            checkpoint["ema_state"] = self.ema.state_dict()
        else:
            decay = self.ema.decay
            trainable = [p for p in self.model.parameters() if p.requires_grad]
            ema_sd = self.ema.state_dict()
            ema_sd["shadow_params"] = [
                (decay * s.detach() + (1.0 - decay) * p.detach().to(s.device)).clone()
                for s, p in zip(ema_sd["shadow_params"], trainable)
            ]
            checkpoint["ema_state"] = ema_sd
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        self.ema.update()
        self._ema_updated_this_batch = True
